<script lang="ts">
    ...
    function handleAIChange(provider: string) {
        selectedAI = provider;
        if (browser) {
    ...
    onMount(() => {
        if (browser) {
            selectedAI = localStorage.getItem("aiProvider") || "gemini";
        }
        (async () => {
            try {
...
<div class="Chat">
    ...
    <main class="main" class:sidebarClosed={!sidebarOpen}>
        <header class="chatTopBar">
            ...
            <div class="aiSelectorContainer">
                <div class="aiSelector">
                    <button 
                        class="aiOption" 
                        class:active={selectedAI === 'gemini'} 
                        on:click={() => handleAIChange('gemini')}
                        title="Usar API do Gemini (Altamente Recomendado)"
                    >
                        <span class="aiOptionIcon">✨</span>
                        <span class="aiOptionName">Gemini API</span>
                        <span class="badge active-badge">Rápido</span>
                    </button>
                    <button 
                        class="aiOption" 
                        class:active={selectedAI === 'ollama'} 
                        on:click={() => handleAIChange('ollama')}
                        title="Usar Ollama Local (Processamento Local)"
                    >
                        <span class="aiOptionIcon">🦙</span>
                        <div class="aiOptionTexts">
                            <span class="aiOptionName">Ollama</span>
                        </div>
                        <span class="badge beta-badge">Beta (Lento)</span>
                    </button>
                </div>
            </div>
        </header>
...
<style>
    ...
    .aiOption:hover .aiOptionIcon {
        transform: scale(1.1);
    }

    .aiOption.active {
        color: #fff;
        box-shadow: 0 4px 12px rgba(144, 19, 254, 0.1);
    }
